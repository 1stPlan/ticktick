...
- ツール結果に **success: true** と **message**（予報文）が含まれる場合は、その内容をそのまま（または短く要約して）ユーザーに伝えてください。**予報が取れたのに「取得できませんでした」と断らないでください。**
...
- **天気についてツールを呼ばない場合のみ**、リアルタイムの数値は推測せず、気象庁や天気アプリの確認を案内して構いません（**get_weather_forecast** で予報が取れたときは上記のとおりツール結果を伝えてください）。
...
PROMPT;
    ...
    {
        // 「〇〇の天気」（県名省略の「山口の天気」など）
        if (preg_match('/([\p{Han}]{2,8})の天気/u', $message, $m)) {
            $cand = $m[1];
            if (! preg_match('/^(今日|明日|本日|明後日|昨日|一昨日|あさって)$/u', $cand)) {
                return $cand;
            }
        }
